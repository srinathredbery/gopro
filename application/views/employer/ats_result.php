                            ...
                            <table class="datatable-init jobpost-table">
                                ...
                                if (isset($active_job_posts) && !empty($active_job_posts)) {
                                    foreach ($active_job_posts as $active_post) {
                                        ...
                                        <?php
                                        $CI =& get_instance();
                                        $CI->load->model('Employer_application_model');

                                       
                                       
                                        //$interviewer=json_decode($interviewer);
                                        //$interviewercount=  count($interviewer['applications']);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for(var i=0;i<data['applications'].length;i++){
                    ...
                    var idats_overall_emp_wise=data['applications'][i]['idats_overall_emp_wise'];
                ...
                if(finalize_status=='Pending'){
                     html+='<td><select id="status"   class="nstatus"'+emp_id+' onchange="changeStatus('+emp_id+','+idats_overall_emp_wise+','+this.value+')"><option value="">Select</option> <option value="Finalized">Finalized</option><option value="Failed">Failed</option>                            </select></td>';
                  }
                  else{
                    
                     html+='<td><select id="status" disabled> <option>'+finalize_status+'</option> </select></td>';
                  }
...
function changeStatus(emp_id,id,status){
             var  emp_id=emp_id
               var id=id;
             var statusn=$('#nstatus'+emp_id).val();;
...
get_white_rice().then(function (rice) {
                ...
                $.ajax({
                    ...
                    url: base_url + "employer/job_posts/ats_result_change_status",
                    data: {'white_rice_token': rice,emp_id:emp_id,id:id,'status':'Finalized'},
                    cache: false,
                    beforeSend: function () {
                        HoldOn.open(loader_options);
                    },
