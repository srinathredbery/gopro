            ...
            <div class="row no-gape">
                ...
                <div class="col-lg-6 column job-sec">
                    <div class="padding-left">
                        <a href="<?php echo base_url()?>employer/job_posts/ats_setup_exam_maker_result">
<!--                            <label><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"-->
<!--                                                                                                                    width="24" height="24"-->
<!--                                                                                                                    viewBox="0 0 172 172"-->
...
<!--                            </label> -->
                            ...
                            <button class="btn btn-back float-left ml-3 mt-3"><i class="fas fa-arrow-left mr-2"></i>Back</button>
                        </a>
                    </div>
                  ...
                  //  data=arrUnique(data);
                    ...
                    for(var i=0;i<data.output.length;i++){
                        ...
                        if(type!=3) {
                            ...
                            Qnumber = Qnumber + 1;
                            Question = data.output[i]['id_ats_exam_question'];
                            if(data.output[i]['isCorrect'] == ''){
                                var iscorrect=0;
                            }
                            else{
                                 var iscorrect=data.output[i]['isCorrect'];
                            }
                            ...
                            html2 +='<div class="col-md-3">Marks<input attr_ip='+ data.output[i]['idats_emp_wise_answer'] +' type="text" min="0" max="'+data.output[i]['mark']+'" name="mark_text"  class="form-control mark_text" placeholder="'+parseFloat(data.output[i]['isCorrect'])+'" value="'+parseFloat(data.output[i]['isCorrect'])+'"/  ></div></div>';
                            // onchange="onClick_test('+ data.output[i]['idats_emp_wise_answer'] +','+ data.output[i]['mark'] +')"

                            html2 += '<div class="col-md-2"><label class="text-danger">Marks : '+data.output[i]['mark']+'</label></div>';

                            html2 += '</div>';

                            total=parseFloat(total)+parseFloat(data.output[i]['isCorrect']);
                            $('#total').val(total);
                        }
                    }
                    ...
                    $("input[type=text]").bind('keyup input', function(){
                        ...
                        $(".mark_text").on('change', function () {
                            ...
                            var pvalue=parseFloat($(this).attr('max'));
                    ...
                    function set_tot(){
                        var t=0;
                        //$(".mark_text").each(function(){

                            $('input[type="text"].mark_text').each(function () {
...
// });
 ...
 get_white_rice().then(function (rice) {
               var formData = new FormData(); // Currently empty
                 ...
                 formData.append('emp_id', '<?php echo $_GET['emp_id'] ?>');
                ...
                $.ajax({
                    type: 'POST',
                    dataType: 'JSON',
                    url: base_url + "employer/employer_post_job/save_total",
