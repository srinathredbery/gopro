...
if (!function_exists('get_number_of_jobs_by_type')) {
    function get_number_of_jobs_by_type($job_type)
    {
        ...
        $CI->db->where('post_status', '1');
          ...
          $CI->db->where('DATE_ADD(job_post_posted_date , INTERVAL 30 DAY) >', "'$today'", false);
...
if (!function_exists('get_number_of_jobs_by_category')) {
    function get_number_of_jobs_by_category($job_cat)
    {
        ...
        $JP->db->where(array('job_post_job_category' => $job_cat, 'post_status'=>'1'));
         $JP->db->where('post_approval', '1');
          $JP->db->where('DATE_ADD(job_post_posted_date , INTERVAL 30 DAY) >', "'$today'", false);
...
if (!function_exists('get_job_post_count')) {
    function get_job_post_count()
    {
        ...
        $CI->db->select("count(*) total_posts,
                           sum(case when DATE_ADD(job_post_posted_date , INTERVAL 30 DAY) >'$today'  and  job_post_posted_date < '$today' and post_status = '0' then 1 else 0 end) inactive_post,
                           sum(case when DATE_ADD(job_post_posted_date , INTERVAL 30 DAY) > '$today'  and  job_post_posted_date < '$today' and post_status = '1' then 1 else 0 end) active_post,
                           sum(case when DATE_ADD(job_post_posted_date , INTERVAL 30 DAY) < '$today' and  (post_status = '0' || post_status = '1' || post_status = '2') then 1 else 0 end) expired_post,
