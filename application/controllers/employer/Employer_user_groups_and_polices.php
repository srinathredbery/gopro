...
class Employer_user_groups_and_polices extends Main_Controller
{
	private $company_id;
	private $user_id;
	...
	public function view_user_group_manager(){
		$data['page_title'] = 'User Group Management';
		$data['user_groups'] = $this->employer_manage_user_model->get_user_groups_list($this->company_id);
		$data['function_routes'] = $this->employer_manage_user_model->get_all_routes();

		$data['main_content'] = 'employer/employer_manage_user_group';
	...
	function save_user_group_access(){
		...
		try {
			...
			$this->employer_manage_user_model->remove_exiting_user_group_permissions($user_group_id);

			foreach ($access_data as $access ){
				$data = array('user_group_id'=>$user_group_id, 'route_id' =>$access);
				$this->employer_manage_user_model->set_user_group_permissions($data);
			}
			if ($this->db->trans_status() === FALSE) {
	...
	function get_existing_permissions(){
		...
		$permission = $this->employer_manage_user_model->get_permissions($group_id);
	...
	function unique_group_name_check($ug_name){
		...
		$is_exist =  $this->employer_manage_user_model->check_user_group_exists($company_id, $ug_name);
