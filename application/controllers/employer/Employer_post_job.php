...
//            redirect('login');
    ...
    public function view_post_editor()
    {
        ...
        if (!empty($selected_post['job_post'])) {
            ...
            $data['questionnaire'] = $this-> employer_job_post_model ->get_questionnaire($post_id);


           // echo var_dump($data['questionnaire']);
...
//save questions to post

    function add_questionnaire()
    {
        $post_id= $this->input->post('post_id');
        $question=$this->input->post('quz');
        ...
        foreach ($question as $quz) {
            $data['question']=$quz;
            $this->employer_job_post_model->add_job_quz($data);
        }
    }
    ...
    function edit_questionnaire()
    {
        ...
        $question=$this->input->post('editquz');

        $updateData=array("question"=>$question);

        $this->db->where("id", $qid);
        $this->db->update("job_questionnaire", $updateData);
    }
    ...
    function ats_post_level_update()
    {
        ...
        $res= $this->employer_job_post_model->ats_post_level_update();
        return $res;
    }
    ...
    function addratings()
    {
        ...
        $data['employer_id']=$employer_id;
         ...
         $this->employer_job_post_model->add_rate($data);
         return '{"success":1}';
    }



//end save question to post
...
//            echo json_encode(array('code' => 449, 'message' => array(validation_errors())));
    ...
    function ats_overall_data_add()
    {
        ...
        $id=$this->input->get('id_shadule');

          $status=$this->input->get('status');
        if ($status=='Selected to 1st round') {
            $data2['level']='1';
        } elseif ($status=='Selected to 2nd round') {
            $data2['level']='2';
        } elseif ($status=='Selected to 3rd round') {
            $data2['level']='3';
        } else {
            $data2['level']='';
        }
        ...
        $level=$this->employer_job_post_model->ats_overall_data_update_level($data2, $id);
...
////        ----------------------------------------------------------------------------
    ...
    function ats_setup_exam_maker_result()
    {
        ...
        $data['active_job_posts'] = $this->employer_job_post_model->get_active_jobs_exame_list($_SESSION['company_id']);
    ...
    {
        $data['page_title'] = 'ATS Result';
        $data['active_job_posts'] = $this->employer_job_post_model->get_active_jobs_onlyschdule($_SESSION['company_id']);
    ...
    function ats_result_change_status()
    {
        $emp_id=$this->input->post('emp_id');
        $id=$this->input->post('id');
        $status=$this->input->post('status');
         ...
         $this->employer_job_post_model->change_status($emp_id, $id, $status);
...
//      $data['active_job_posts'] = $this->employer_job_post_model->get_active_jobs($_SESSION['company_id']);
        $data['active_job_posts'] = $this->employer_job_post_model->get_active_jobs_schedule($_SESSION['company_id'], $_GET['id']);
    ...
    function ats_interviewee_form_maker_view()
    {
        ...
        $data['active_job_posts'] = $this->employer_job_post_model->ats_interviewee_form_view($_SESSION['company_id']);
         $data['marks'] =$this->employer_job_post_model->ats_interviewee_form_view_marks($id);
        $data['main_content'] = 'employer/ats_interviewee_form_maker_view';

       
        //load main content page;
        $data['resume_id'] = $this->employer_job_post_model->ats_resume_info($id);
     //   $data['marks'] = $this->employer_job_post_model->getpercentageavg($id);
...
//ats_exam_save
//      $job_post_form['is_delete']=0;
        foreach ($this->input->get('interviewer') as $value) {
             $job_post_form['interviewer']= $value;
                $job_post_form['confirmation_link']= $value;
...
//      {
        ...
        if ($id_list==null || $id_list=="") {
            ...
            $job_post_form['user_id'] = $_SESSION['user_id'];
            $job_post_form['email'] = $this->input->get('contact_interviewer');
...
//              $job_tag['id_ats_interviewee_form_maker'] = $this->employer_job_post_model->ats_interviewer_form_update($job_tag['id_ats_interviewee_form_maker'], $job_post_form);
    ...
    function get_data_emp_mcq()
    {
        ...
        //$count=get_data_emp_mcq($emp_id, $examIDD);
               ...
               $count=$this->employer_job_post_model->getmcqmarks($emp_id, $examIDD);
    ...
    function get_data_emp_short_answer()
    {
       ...
       // $count=get_data_emp_short_answer($emp_id, $examIDD);
               ...
               $count=$this->employer_job_post_model->getshortmarks($emp_id, $examIDD);
    ...
    function get_tot_percentage()
    {
        ...
        $count=$this->employer_job_post_model->getpercentage($emp_id, $examIDD);
...
//---------------------------------------------------------------------------------------
        ...
        $data['skills'] = $this->employer_application_model->get_job_post_skills($id);
        $data['questions'] = $this->employer_application_model->get_job_post_questions($id);
        $data['quizrate'] = $this->employer_application_model->get_job_post_quizrate($id);
        

        
      //  $data['ratings'] = $this->employer_application_model->get_job_post_rating($id);
       ...
       // $data['applications'] = $this-> employer_application_model-> get_recieved_applications($_SESSION['company_id'], $id);
        $data['applications'] = $this-> employer_application_model-> get_recieved_applicationsats($_SESSION['company_id'], $id);


         $data['applications_cart'] = $this-> employer_application_model-> get_recieved_applicationsatscart($_SESSION['company_id'], $id);



        $data['applications_candidates'] = $this-> employer_application_model-> get_candidates($_SESSION['company_id'], $id);
...
////        ----------------------------------------------------------------------------
    ...
    function ats_exam_schedule_emp_view()
    {
        ...
        $data['applications'] = $this-> employer_application_model-> get_recieved_applications_emp_exam_schedule($_SESSION['user_id'], $id);
         ...
         $data['exam_levels'] = $this-> employer_application_model-> get_levels($id);
