...
//            redirect('login');
    ...
    public function view_job_seeker_profile()
    {
        ...
        if (base64_decode($resume_id, true) === false) {
            $resume_id = $this->input->get('r_id');
        }
...
//get ratings
        ...
        $sql="select ratings from job_applications_received where job_post_id='$job_id' and  applied_resume='$resume_id'";
        $query=$this->db->query($sql);
        $result= $query->result();
        if ($query->num_rows() >0) {
            foreach ($query->result() as $row) {
                $data['rate']=$row->ratings;
            }
        } else {
            $data['rate']='Unrated';
        }
...
//
        ...
        //Resume Info
        $resume_info = $this->resume_model->get_resume_info_byid(array('resume_id' => $resume_id));
