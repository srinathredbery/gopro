...
class Rat
{
    ...
    public function log($message, $code = 0, $user_id = 0)
    {
        ...
        $session_user_type = $this->ci->config->item('session_user_type','rat');
		$user_type = isset($_SESSION[$session_user_type]) ? $_SESSION[$session_user_type] : '0';
        if(($user_id==0) && !empty($session_user_id))
        {
            $user_id = isset($_SESSION[$session_user_id]) ? $_SESSION[$session_user_id] : '0';
        }

        if($this->_set_message($message,$user_id, $user_type,$code))
        {
            return TRUE;
        }
        else
    ...
    private function _set_message($message,$user_id,$user_type,$code)
    {
