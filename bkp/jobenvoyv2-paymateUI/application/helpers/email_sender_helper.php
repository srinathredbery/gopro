...
if (!function_exists('send_welcome_mail_jobseeker')) {
    function send_welcome_mail_jobseeker($user_mail)
    {
        ...
        */
        ...
        $CI->email->from('omrangroup@mail.com', 'omrangroup.com');
        $CI->email->to($user_mail, $user_mail);
        $CI->email->subject('Get started with your career');
        $CI->email->message(WELCOME_MAIL);
        try {
            $CI->email->send();
...
if (!function_exists('send_password_changed_email')) {
    function send_password_changed_email($user_mail, $message)
    {
        ...
        $CI->email->from('omrangroup@mail.com', 'omrangroup.com');
        $CI->email->to($user_mail, $user_mail);
        $CI->email->subject('Your Jobenvoy account password has been changed');
